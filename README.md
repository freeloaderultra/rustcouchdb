...
| [`couch-store`](couch-store/) | Storage engine for `.couch` shard files — couch_file / couch_btree / couch_key_tree ported to Rust, read **and** write. | verified bidirectionally against CouchDB 3.5.1 (server opens, updates, compacts and replicates out of Rust-written files); bulk ingest ~2.5× the server's HTTP rate |

Both crates build standalone with `cargo build --release` (rustls only, no
